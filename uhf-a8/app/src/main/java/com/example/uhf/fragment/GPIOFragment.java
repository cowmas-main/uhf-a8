...
public class GPIOFragment extends KeyDownFragment implements View.OnClickListener {
    ...
    public void InputClick(View view)  {
        int[] Data= new int[0];
        try {
            Data = RFIDWithUHFA8.getInstance().inputStatus();
        } catch (ConfigurationException e) {
            e.printStackTrace();
        }
        if(Data==null){
            ...
            AlertDialog alertDialog1 = new AlertDialog.Builder(getActivity())
                    .setTitle("gpio")//标题
                    .setMessage("input1: "+Data[0] +"  input2: "+Data[1])//内容
                    .create();
            alertDialog1.show();
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.btnInputStatusA4:
                int[] Data=rfidWithUHFA4.inputStatus();
                if(Data==null){
                    Toast.makeText(getActivity(),"获取失败",Toast.LENGTH_SHORT).show();
                }else {
                    AlertDialog alertDialog1 = new AlertDialog.Builder(getActivity())
                            .setTitle("gpio")//标题
                            .setMessage("input1: "+Data[0] +"  input2: "+Data[1]+" \r\ninput3: "+Data[2] + "  input4:" +Data[3])//内容
                            .create();
                    alertDialog1.show();
